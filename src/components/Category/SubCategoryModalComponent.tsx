...
const SubCategoriesModal: React.FC<SubCategoriesModalProps> = ({
  ...
  useEffect(() => {
    if (open && parentCategoryIds.length) {
      dispatch(
        fetchSubCategories({
          page,
          pageSize: PAGE_SIZE,
  ...
  // Reset selected subcategory IDs when modal opens or selection changes from parent
  useEffect(() => {
    if (open) setSelectedIds(selectedSubCategoryIds);
  }, [open, selectedSubCategoryIds]);
